...
RUN echo -ne "load_module /usr/lib/nginx/modules/ngx_http_brotli_static_module.so;\nload_module /usr/lib/nginx/modules/ngx_http_brotli_filter_module.so;\n" | cat - /etc/nginx/nginx.conf >> /etc/nginx/nginx.conf_tmp
RUN mv /etc/nginx/nginx.conf_tmp /etc/nginx/nginx.conf

RUN apk add --update inotify-tools

COPY ./nginx/docker-nginx-reloader.sh /usr/local/bin/docker-nginx-reloader.sh
COPY ./nginx/docker-entrypoint.sh /usr/local/bin/docker-entrypoint.sh

RUN chmod +x /usr/local/bin/docker-nginx-reloader.sh
